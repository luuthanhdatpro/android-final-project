...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public int countEssQuestion(){
        openDatabase();
        Cursor cursor = mDatabase.rawQuery("SELECT COUNT(*) FROM Questions WHERE  isEssential = 1;", null);
        int count = 0;
        if(cursor != null){
            if(cursor.getCount() > 0){
                cursor.moveToFirst();
    ...
    public int countDoneEssQuestion(){
        openDatabase();
        Cursor cursor = mDatabase.rawQuery("SELECT COUNT(*) FROM Questions WHERE  isEssential = 1 AND isLearned = 1;", null);
        int count = 0;
        if(cursor != null){
            if(cursor.getCount() > 0){
                cursor.moveToFirst();
    ...
    public ArrayList<Question> getEssQuestion(){
        openDatabase();
        Cursor cursor = mDatabase.rawQuery("SELECT * FROM Questions WHERE isEssential=1;",null);
        ArrayList<Question> qList = new ArrayList<>();
        while (cursor.moveToNext()){
            int id = cursor.getInt(0);
            String title = cursor.getString(1);
